...
// GET /tasks?sortBy=createdBy:asec
...
router.patch("/tasks/:id", auth, async (req, res) => {
  ...
  try {
    const task = await Task.findOne({ ...req.params._id, owner: req.user._id });

    if (!task) {
      return res.status(404).send();
    }

    updateOperations.forEach((update) => (task[update] = req.body[update]));
    await task.save();
